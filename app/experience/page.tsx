    ...
    <>
      <section className="py-24 md:py-32 px-4 md:px-6 min-h-screen bg-white/5">
        <div className="container mx-auto max-w-6xl">
            ...
            {experiences.map((exp, index) => {
              ...
              return (
                <motion.div
                  ...
                  className="glass rounded-xl p-5 md:p-6 hover:bg-white/10 transition-all"
                >
                    ...
                    <div>
                      ...
                      {exp.companyUrl ? (
                        <a
                          ...
                          className="text-lg md:text-xl text-primary hover:text-accent transition-colors mb-1 inline-flex items-center gap-2 group"
                        >
                          {exp.company}
                          <ExternalLink size={16} className="group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform" />
                        </a>
                      ) : (
                        <p className="text-lg md:text-xl text-primary mb-1">{exp.company}</p>
                      )}
                  ...
                  <p className="text-sm md:text-base text-foreground/80 mb-4">{exp.description}</p>
                  ...
                  {isExpanded && (
                    ...
                    >
                      <h4 className="text-base md:text-lg font-semibold mb-3 text-accent">Accomplishments</h4>
                      <div className="space-y-2 mb-4">
                        {exp.achievements.map((achievement, idx) => (
                          <p key={idx} className="text-sm md:text-base text-foreground/70 flex items-start gap-2">
          ...
          {/* Education Section */}
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold mb-6 md:mb-8 text-center text-gradient">
            Education
          </h2>
          <motion.div
          ...
          >
            <div className="flex items-start gap-4 mb-4">
              ...
              <div className="flex-1">
                <h3 className="text-lg md:text-xl font-bold mb-1">{education.degree}</h3>
                <p className="text-sm md:text-base text-primary mb-1">{education.institution}</p>
                <div className="flex flex-wrap gap-3 md:gap-4 text-xs md:text-sm text-foreground/60">
                ...
                {showEducationDetails && education.description && (
                  <motion.div
                    ...
                    className="mt-4"
                  >
                    <h4 className="text-base md:text-lg font-semibold mb-2 text-accent">Accomplishments</h4>
                    <p className="text-sm md:text-base text-foreground/70">{education.description}</p>
                  </motion.div>
                )}
              </div>
            </div>
